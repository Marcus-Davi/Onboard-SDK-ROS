...
void DJISDKNode::dataBroadcastCallback()
{
  ...
  uint16_t flag_has_rc = isM100() ? (data_enable_flag & DataBroadcast::DATA_ENABLE_FLAG::HAS_RC) : (data_enable_flag & DataBroadcast::DATA_ENABLE_FLAG::A3_HAS_RC);
  if (flag_has_rc)
  {
  ...
  uint16_t flag_has_battery =
      isM100() ? (data_enable_flag & DataBroadcast::DATA_ENABLE_FLAG::HAS_BATTERY) : (data_enable_flag & DataBroadcast::DATA_ENABLE_FLAG::A3_HAS_BATTERY);
  ...
  uint16_t flag_has_status =
      isM100() ? (data_enable_flag & DataBroadcast::DATA_ENABLE_FLAG::HAS_STATUS) : (data_enable_flag & DataBroadcast::DATA_ENABLE_FLAG::A3_HAS_STATUS);
  ...
  uint16_t flag_has_gimbal =
      isM100() ? (data_enable_flag & DataBroadcast::DATA_ENABLE_FLAG::HAS_GIMBAL) : (data_enable_flag & DataBroadcast::DATA_ENABLE_FLAG::A3_HAS_GIMBAL);
  if (flag_has_gimbal)
  {
...
void DJISDKNode::publish5HzData(Vehicle *vehicle, RecvContainer recvFrame,
                                DJI::OSDK::UserData userData)
{
  ...
  // FIXME por alguma razao, tá publicando sempre 0 ... Ver em campo
  p->gps_raw_position_publisher.publish(gps_raw);
